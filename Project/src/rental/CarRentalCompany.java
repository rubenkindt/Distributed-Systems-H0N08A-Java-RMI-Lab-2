...
public class CarRentalCompany implements InterfaceCarRentalCompany{
	...
	public List<Reservation> getReservationsByRenter(String clientName){
		...
		for (int car =0;car<cars.size();car++) {
			...
			for (int j=0;j<cars.get(car).GetReservations().size();j++){
				
				if (cars.get(car).GetReservations().get(j).getCarRenter()==clientName) {
					
					resList.add(cars.get(car).GetReservations().get(j));
				}
			}
		}
